  ...
  db.check_user_exists(email).then(user => {
    ...
    bcrypt.genSalt(saltRounds).then(salt => {
      ...
      bcrypt.hash(password, salt).then(hashedPassword => {
        // insert the email address and the hasshed password into the database
        db.create_user([email, hashedPassword]).then(createdUser => {
          // return newly created user and set them to a session.user so they can begin thier unique user experience
          req.session.user = {
            id: createdUser[0].id,
